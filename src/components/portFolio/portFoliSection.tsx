...
const PortFolioSection: FC<PortFolioSectionProps> = () => {
    ...
    const filterHandler = (data: string) => {
        if (data === "all") {
        ...
        } else if (data === "wordpress" || data === "wordpress-ecommerce" || data === "wordpress-blog" || data === "wordpress-others") {
            setNum(1)
            if (data === "wordpress") {
                const filterDataWordpress = allDataReciev.filter(product => product.category === data)
                setData(filterDataWordpress)
            }
            else if (data === "wordpress-ecommerce") {
                const fileDataEcommerce = allDataReciev.filter(product => product.subCategory === data)
                setData(fileDataEcommerce)
            }
            else if (data === "wordpress-blog") {
                const fileDataBlog = allDataReciev.filter(product => product.subCategory === data)
                setData(fileDataBlog)
            }
            else if (data === "wordpress-others") {
                const fileDataOthers = allDataReciev.filter(product => product.subCategory === data)
            ...
            setNum(0)
            const filterData = allDataReciev.filter(product => product.category === data)
            setData(filterData)
        }
    }
    return (
