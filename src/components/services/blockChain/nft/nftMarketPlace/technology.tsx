...
const allImageData: imageList[] = [{ image: sImg },
{ image: reactjs },
{  image: reactnative },
{  image: jsimg },
{  image: angular },
...
const TechnologySection:FC<technologyProps> = () => {
    return (<div className="pt-16">

<p className=" text-2xl md:text-4xl font-bold text-center">Technologies We Use</p>
...
<div className="flex flex-wrap justify-center max-w-4xl items-center gap-16 pt-12   mx-8 md:mx-24 lg:mx-64 pb-20">
                {allImageData.map(img => {
                    return <div key={Math.random()} className=" flex items-center justify-end">
                        <Image src={img.image.src} alt="" width={200} height={200} className="md:w-28 md:h-22 w-24 h-16 " />
                    </div>
                })
            }
        </div>
